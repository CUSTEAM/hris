...
public class PrintAction extends BaseAction{
	...
	public String StayTimePrint() throws IOException{
		...
		Date staytime_begin=(Date)getContext().getAttribute("date_staytime_begin");
		Date staytime_end=(Date)getContext().getAttribute("date_staytime_end");		
		
		//修改次數
		List<Map>edit=df.sqlGet("SELECT (SELECT COUNT(*)FROM Empl_stay_hist WHERE idno=e.idno AND edate>='"+getContext().getAttribute("staytime_end")+"')as cnt, e.cname, cd.name as dname, "
		...
		//未填寫
		List<Map>none=df.sqlGet("SELECT e.cname, e.sname, cd.name as dname, e.CellPhone, e.Email FROM"
		+ "(empl e LEFT OUTER JOIN CODE_DEPT cd ON e.unit=cd.id) WHERE e.category='1'AND "
		+ "e.idno NOT IN(SELECT idno FROM Empl_stay_info WHERE school_term='"+getContext().getAttribute("school_term")+"' AND school_year='"+getContext().getAttribute("school_year")+"')ORDER BY e.unit");
		
		//不足
		List<Map>enough=df.sqlGet("SELECT e.cname, e.sname, d.name as dname, e.CellPhone, e.Email,"
				+ "IFNULL(time_stay,0)as t, IFNULL(( SELECT time FROM Empl_techlimit WHERE idno=et.idno),0)as s,"
				+ "(SELECT COUNT(*)FROM Empl_stay_info WHERE idno=e.idno AND school_term='"+getContext().getAttribute("school_term")+"' AND school_year='"+getContext().getAttribute("school_year")+"')as r "
				+ "FROM (empl e LEFT OUTER JOIN Empl_techlimit et ON e.idno=et.idno)"
				+ "LEFT OUTER JOIN CODE_DEPT d ON d.id=e.unit WHERE e.category='1'");		
		
		//星期
		List<Map>empls=df.sqlGet("SELECT e.idno, cd.name as dname, e.cname, esp.contact, e.CellPhone, esp.place FROM"
		+ "(empl e LEFT OUTER JOIN Empl_stay_place esp ON e.idno=esp.idno)"
		+ "LEFT OUTER JOIN CODE_DEPT cd ON e.unit=cd.id WHERE e.category='1'ORDER BY e.unit");
		
		StayTimePrint p=new StayTimePrint(response, none, edit, empls, enough, df.sqlGet("SELECT * FROM Empl_stay_info ORDER BY week, period"), info);
		p.print();
